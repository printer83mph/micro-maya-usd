...
void OpenGLContext::debugContextVersion() {
  ...
  if (ctxmajor < 3 || glsl.startsWith("1.10") || glsl.startsWith("1.20")) {
    printf("ERROR: "
           ...
           "If you have switchable graphics, make sure that you are using the "
           "discrete GPU.\n");
    QApplication::exit();
    ...
    printf("WARNING: "
           ...
           "If you have switchable graphics, make sure that you are using the "
           "discrete GPU. "
           "If you cannot get 3.2 support, it is possible to port this "
           "project....");

    // Note: doing this requires at least the following actions:
    // * Change the header and base class in glwidget277.h to 3.0/3.1 instead
    // of 3.2 Core.
