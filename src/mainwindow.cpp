...
MainWindow::MainWindow(QWidget *parent)
  ...
  // load OBJ button
  connect(ui->actionImportOBJ, &QAction::triggered, this,
  ...
  connect(ui->mygl, &MyGL::signal_addVertex, this, &MainWindow::slot_addVertex);
  connect(ui->mygl, &MyGL::signal_addFace, this, &MainWindow::slot_addFace);
  connect(ui->mygl, &MyGL::signal_addEdge, this, &MainWindow::slot_addEdge);
  connect(ui->mygl, &MyGL::signal_setJoint, this, &MainWindow::slot_setJoint);

  // selecting vert, face, edge, or joint
  connect(ui->vertsListWidget, &QListWidget::itemClicked, this,
          &MainWindow::slot_setSelectedVertex);
  connect(ui->facesListWidget, &QListWidget::itemClicked, this,
          &MainWindow::slot_setSelectedFace);
  connect(ui->halfEdgesListWidget, &QListWidget::itemClicked, this,
          &MainWindow::slot_setSelectedEdge);
  connect(ui->jointsTreeWidget, &QTreeWidget::itemClicked, this,
          &MainWindow::slot_setSelectedJoint);
  // from mygl to here back to mygl
  connect(ui->mygl, &MyGL::signal_setSelectedVertex, this,
          &MainWindow::slot_setSelectedVertex);
  connect(ui->mygl, &MyGL::signal_setSelectedFace, this,
          &MainWindow::slot_setSelectedFace);
  connect(ui->mygl, &MyGL::signal_setSelectedEdge, this,
          &MainWindow::slot_setSelectedEdge);

  // set vertex position, face color
  connect(ui->vertPosXSpinBox, &QDoubleSpinBox::valueChanged, ui->mygl,
          &MyGL::slot_setVertPosX);
  connect(ui->vertPosYSpinBox, &QDoubleSpinBox::valueChanged, ui->mygl,
          &MyGL::slot_setVertPosY);
  connect(ui->vertPosZSpinBox, &QDoubleSpinBox::valueChanged, ui->mygl,
          &MyGL::slot_setVertPosZ);

  const float rotIncrement = 0.125f * PI;
  // update joint rotation
  connect(ui->jointRotXBackButton, &QPushButton::released, ui->mygl,
          [=] { ui->mygl->rotateJoint(-rotIncrement, 0, 0); });
  connect(ui->jointRotXForButton, &QPushButton::released, ui->mygl,
          [=] { ui->mygl->rotateJoint(rotIncrement, 0, 0); });
  connect(ui->jointRotYBackButton, &QPushButton::released, ui->mygl,
          [=] { ui->mygl->rotateJoint(0, -rotIncrement, 0); });
  connect(ui->jointRotYForButton, &QPushButton::released, ui->mygl,
          [=] { ui->mygl->rotateJoint(0, rotIncrement, 0); });
  connect(ui->jointRotZBackButton, &QPushButton::released, ui->mygl,
          [=] { ui->mygl->rotateJoint(0, 0, -rotIncrement); });
  connect(ui->jointRotZForButton, &QPushButton::released, ui->mygl,
          [=] { ui->mygl->rotateJoint(0, 0, rotIncrement); });

  connect(ui->faceRedSpinBox, &QDoubleSpinBox::valueChanged, ui->mygl,
          &MyGL::slot_setFaceRed);
  connect(ui->faceGreenSpinBox, &QDoubleSpinBox::valueChanged, ui->mygl,
          &MyGL::slot_setFaceGreen);
  connect(ui->faceBlueSpinBox, &QDoubleSpinBox::valueChanged, ui->mygl,
          &MyGL::slot_setFaceBlue);

  // mesh operations
  connect(ui->splitEdgeButton, &QPushButton::released, ui->mygl,
          &MyGL::slot_splitEdge);
  connect(ui->triangulateFaceButton, &QPushButton::released, ui->mygl,
          &MyGL::slot_triangulateFace);
  connect(ui->subdivideMeshButton, &QPushButton::released, ui->mygl,
          &MyGL::slot_subdivideMesh);

  // skeleton operations
  connect(ui->bindMeshButton, &QPushButton::released, ui->mygl,
          &MyGL::bindMesh);
}
