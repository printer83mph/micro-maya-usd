...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      attrPos(-1), attrNor(-1), attrCol(-1), attrJointIdx(-1), attrJointWgt(-1),
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifCamPos(-1),
      unifBindMats(-1), unifJointTfms(-1),
      context(context)
{}
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (attrJointWgt != -1 && d.bindJointWgt()) {
        ...
        context->glVertexAttribPointer(attrJointWgt, 2, GL_FLOAT, false, 0, nullptr);
    }
    ...
    if (attrJointIdx != -1) context->glDisableVertexAttribArray(attrJointIdx);
    if (attrJointWgt != -1) context->glDisableVertexAttribArray(attrJointWgt);
