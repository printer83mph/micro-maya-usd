...
void Mesh::create()
{
    ...
    for (auto& face : faces) {
        ...
        do {
            ...
            nor.push_back(glm::vec4(glm::normalize(glm::cross(currVert - prevVert,
                                                              nextVert - currVert)), 0));
            ...
            if (isBound())
            {
                ids.push_back(glm::ivec2(currEdge->nextVert->joint1Idx, currEdge->nextVert->joint2Idx));
                weights.push_back(glm::vec2(currEdge->nextVert->joint1Weight, currEdge->nextVert->joint2Weight));
            }
    ...
    if (isBound())
    {
        ...
        mp_context->glBufferData(GL_ARRAY_BUFFER, ids.size() * sizeof(glm::ivec2), &ids[0], GL_STATIC_DRAW);

        generateJointWgt();
        mp_context->glBindBuffer(GL_ARRAY_BUFFER, bufJointWgt);
        mp_context->glBufferData(GL_ARRAY_BUFFER, weights.size() * sizeof(glm::vec2), &weights[0], GL_STATIC_DRAW);
    }
}
...
{
    return splitEdge(edge, (edge->nextVert->pos + edge->sym->nextVert->pos) * 0.5f);
}
...
void Mesh::triangulateFace(Face* face) {
    ...
    for (int i = 0; i < edgeCount - 3; ++i)
    {
        ...
        Vertex* connectVert = edge->nextVert;  // vert these specific new edges attach to
...
void Mesh::catmullClarkSubdivide()
{
    ...
    for (int ei = 0; ei < initialHalfEdgeCount; ++ei)
    {
        ...
        glm::vec3 faceSum = centroids[edge->face]->pos +
                (hasTwoFaces ? centroids[edge->sym->face]->pos : glm::vec3(0));

        glm::vec3 midpointPos = (edge->getHeadPos() + edge->getTailPos() + faceSum) /
                (hasTwoFaces ? 4.f : 3.f);
    ...
    for (int vi = 0; vi < initialVertCount; ++vi)
    {
        ...
        vert->pos = ((adjMidpointCount - 2.f) / (float) adjMidpointCount) * vert->pos +
                         ...
                         const std::vector<std::vector<int>>& fileFaces)
{
    ...
    // hash map bullshit
    std::unordered_map<std::pair<Vertex*, Vertex*>, HalfEdge*, PairHash> edgeIndices;
...
void Mesh::quadrangulateFace(Face* face, Vertex* centroid, std::vector<HalfEdge*>& splitEdges)
{
    ...
    for (unsigned int ei = 0; ei < splitEdges.size(); ++ei)
    {
        ...
        newOutEdge->nextEdge = lastEdges[((int) ei - 1 + splitEdges.size()) % splitEdges.size()];

        newInEdge->nextVert = centroid;
        newOutEdge->nextVert = splitEdges[((int) ei - 1 + splitEdges.size()) % splitEdges.size()]->nextVert;
    ...
    for (unsigned int ei = 0; ei < splitEdges.size(); ++ei)
    {
        ...
        HalfEdge* outEdge = splitEdges[((int) ei + 1) % splitEdges.size()]->nextEdge->nextEdge;
    ...
    for (unsigned int i = 1; i < splitEdges.size(); ++i) {
        glm::vec3 newCol = glm::clamp(face->color + (utils::getRandomColor() * 0.3f - 0.15f),
